 ...
 * 返回: 成功，UNC_AE_READABLE/UNC_AE_WRITEABLE.失败，0
 **/
 ...
 *      3. time_proc函数中可能会注册新的时间事件，为了防止unc_process_time_events
...
int unc_ae_process_events(unc_ae_event_loop *el, int flags) 
{
        ...
        ((flags & UNC_AE_TIME_EVENTS) && !(flags & UNC_AE_DONT_WAIT))) 
    {
        ...
        {
            /* 如果没有时间事件，不推荐!! */ 
            /* 默认情况下UNC_AE_DONT_WAIT未设置，poll会一直阻塞直到file事件触发，
        ...
        /* 处理完所有的file事件(在函数unc_ae_api_poll中，fired[]是不断覆盖的) */
        for (j = 0; j < numevents; ++j) 
        {
